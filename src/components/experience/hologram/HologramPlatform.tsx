...
export function HologramPlatform(){
 ...
 useFrame((_,delta)=>{const s=useExperience.getState();if(s.paused||!root.current)return;if(replay.current!==s.replayVersion){replay.current=s.replayVersion;time.current=0;}time.current+=Math.min(delta,.05)*(reducedMotion()?.08:1);if(ring.current)ring.current.rotation.z=time.current*.15;if(outer.current)outer.current.rotation.z=-time.current*.08;root.current.position.x=THREE.MathUtils.damp(root.current.position.x,(s.currentScene==='website'?-2.05:0)+parallax.x*.1,4,delta);const scale=s.started?THREE.MathUtils.smoothstep(s.time,1.6,2.4):0;root.current.scale.setScalar(scale*(s.currentScene==='website'?.8:1));if(light.current)light.current.opacity=.05+audioSync.state.audioAmplitude*.08+Math.sin(time.current*1.3)*.015;});
