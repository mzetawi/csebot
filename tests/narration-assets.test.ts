...
test('active Arabic script covers the minute tour and uses the supplied image',()=>{
 ...
 expect(manifest.language).toMatch(/^ar/);expect(manifest.provider).toBe('custom');
 for(const scene of scenes.filter(s=>s.lines.length)){expect(statSync(resolve('public',`.${manifest.clips[scene.id]}`)).size).toBeGreaterThan(1000);}
